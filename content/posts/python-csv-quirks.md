...
One solution would've been to update the PostgreSQL table definition to allow null values for column `c`. Unfortunately that wasn't an option at the time because there were too many sql transformation and cross-functional dependencies on that column as it was currently defined.
